...
class BitbucketIssueSync(IssueSyncMixin):
    description = 'Integrate Bitbucket issues by linking a repository to a project.'
    slug = 'bitbucket'
    conf_title = 'Bitbucket'
    conf_key = 'bitbucket'

    def get_group_urls(self):
        return super(BitbucketIssueSync, self).get_group_urls() + [
            (
    ...
    def message_from_error(self, exc):
        if isinstance(exc, ApiError) and exc.code == 404:
            return ERR_404
        return super(BitbucketIssueSync, self).message_from_error(exc)
