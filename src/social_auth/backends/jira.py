...
JIRA_SERVER = 'bitbucket.org/api/1.0'
JIRA_REQUEST_TOKEN_URL = '{host}/plugins/servlet/oauth/request-token'
JIRA_ACCESS_TOKEN_URL = '{host}/plugins/servlet/oauth/access-token'
JIRA_AUTHORIZATION_URL = '{host}/plugins/servlet/oauth/authorize'
# BITBUCKET_EMAIL_DATA_URL = 'https://%s/emails/' % BITBUCKET_SERVER
# BITBUCKET_USER_DATA_URL = 'https://%s/users/' % BITBUCKET_SERVER


class JiraBackend(OAuthBackend):
    ...
    def tokens(cls, instance):
        ...
        """
        token = super(JiraBackend, cls).tokens(instance)
...
class JiraAuth(ConsumerBasedOAuth):
